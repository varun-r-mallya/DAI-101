...
# The dropna method removes any NaN values from the 'Age' column, 
# to_numpy converts the column to a NumPy array.

column_np = df[column].dropna().to_numpy()

print(f"column_np:{column_np}")
...
# df_cleaned_zscore, contains all rows that are not considered outliers according to the Z-score method, plus any rows with missing values in the specified column.
